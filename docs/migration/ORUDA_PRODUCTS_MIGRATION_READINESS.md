...
| VS Code package | `(cd vscode-extension && npm ci --ignore-scripts --no-audit --no-fund && npm test && npm run package)` | `PASS_NONFINAL` (10 Node tests, seven-group universal 명령과 Gateway 운영 View, VSIX SHA-256 `30d27a88c424...`; proprietary LICENSE/third-party notice 포함) |
| VS Code Extension Host | `python3 scripts/rehearse_vscode_extension_host.py` | `PASS_NONFINAL` (Node 22, VS Code 1.95.3/Xvfb, 양쪽 extension host exit 0, engine mismatch 경고 0, offline network 차단, source/image/log digest receipt) |
...
`THIRD_PARTY_NOTICES.md`와 번들 JAR에서 추출한 upstream license 원문 2개를 포함한다.
Ubuntu 후보는 AppArmor profile/drop-in을 포함한 37개 파일/36개 내부 checksum에 결속한다.
현재 RHEL SHA-256은 `74ec4e2c844f...`, Ubuntu는 `84098d26467a...`다. 이 소유자
