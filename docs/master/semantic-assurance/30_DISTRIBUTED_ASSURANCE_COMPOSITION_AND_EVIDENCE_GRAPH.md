...
Parent: `29_DEPLOYMENT_RUNTIME_CURRENTNESS_AND_REVOCATION_DESIGN.md`
...
### 5.3 N/A
N/A는 applicability rule id, evaluated subject digest, rationale, evaluator identity, evidence digest, challenge/review status를 요구한다.

### 5.4 Conflicting Results
동일 claim/context에서 PASS와 FAIL이 공존하면 최신 결과만 자동 선택하지 않는다. supersession relation이 증명되면 superseding result를 사용하고, 아니면 `CONFLICT_HOLD`다.

### 5.5 Retry
Retry PASS가 이전 실패를 삭제하지 않는다. Attempt graph를 유지하고 실패의 disposition receipt가 있어야 한다.
...
Agent가 Product 핵심 기능의 HARD dependency이면 Product PASS 금지. LLM Provider가 STALE이면 Product currentness도 CURRENT 불가. 결과 예: `decision=HOLD, assurance_level=AL2, currentness=REASSESSMENT_REQUIRED`.

## 7. Evidence Graph
Node 종류: Source, Requirement, Policy, Oracle, Fixture, Execution, Observation, Finding, RCA, Patch, Approval, Qualification, EvidenceReceipt, FinalLock, Deployment, RuntimeObservation, Certificate.
...
- INVALIDATES/REVOKES는 node 삭제가 아니라 validity generation 변경을 생성.
- FinalLock/Certificate는 material parent를 graph에서 재구성 가능해야 함.
- dangling edge 금지.
- tenant 간 edge 금지. cross-tenant public authority는 별도 namespace 사용.

## 9. Composition Engine
입력: exact subject population, exact dependency edge population, applicable claims/requirements, current evidence graph head, current policy/composition rule version.

처리: population lock → applicability closure → topology validation → freshness validation → contradiction resolution → hard dependency propagation → assurance level ceiling → currentness ceiling → product result generation.

출력: CompositionSnapshot, CompositionReceipt, unresolved conflict list, ceiling explanation graph.
...
## 11. UI/UX
제품 Dashboard는 단일 녹색 PASS 대신 Product decision, Assurance Level, Currentness, Critical dependency 상태, weakest required child, unresolved conflict, excluded/N/A population, last composition time을 함께 표시한다.

## 12. Negative Test
- 동일 child 중복 denominator
- critical child soft 위장
- N/A rationale 없는 제외
...
- `08`: Assurance Level naming, hard/soft dependency governance

## 15. 비최종 경계
현재 Claude 개발 Batch의 active contract가 아니다. 후속 Contract/Fixture/Runtime 설계 후 별도 Handoff한다.
