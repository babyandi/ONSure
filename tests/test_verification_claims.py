...
class VerificationClaimFailureInjectionTest(unittest.TestCase):
    def run_case(
        ...
        count_authority: dict | None = None,
        add_workflow: bool = False,
        local_gate: str = LOCAL_GATE,
    ) -> list[str]:
    ...
    def test_failure_injection_total_cannot_stay_at_old_baseline(self):
        status = safe_status()
        status["omission_failure_injection"]["all_registered_failure_injections"] = 87
    ...
    def test_failure_injection_count_contract_must_balance(self):
        authority = json.loads(json.dumps(COUNT_AUTHORITY))
        authority["total"] = 90
        self.assertIn("FAILURE_COUNT_AUTHORITY_TOTAL_MISMATCH",
                      self.run_case(count_authority=authority))

    def test_worktree_authority_discovery_cannot_be_removed(self):
        status = safe_status()
        status["approval_authority_boundary"].pop("contained_worktree_authority_discovery")
