 ...
 * It exposes stored facts and explicit availability only; it does not infer assurance progression.
 */
public final class EnterpriseWebReadService {
    ...
    public record TargetSummary(
            ...
            String canonicalAssuranceState,
            long coreRevision) {}
    ...
    public EnterpriseWebReadService(Path catalogRoot, Path validationRoot) {
        this.catalog = new ProductCatalog(catalogRoot);
    ...
    public List<ProjectSummary> projects() throws Exception {
        ...
        for (Project project : catalog.projects()) {
            values.add(new ProjectSummary(
                    project.projectId(), project.workspaceId(), project.name(),
    ...
    public List<TargetSummary> targets(String projectId) throws Exception {
        ...
        return catalog.targets(projectId).stream()
                .map(value -> targetSummary(value, revision))
                .toList();
    }
    ...
    public AssuranceSnapshot assurance(String projectId, String targetId) throws Exception {
        ...
        return new AssuranceSnapshot(
                targetId,
                Availability.NOT_AVAILABLE,
                null,
                "CORE_ASSURANCE_PROJECTION_NOT_IMPLEMENTED",
                catalog.revision());
    }
    ...
    public List<EvidenceReceipt> evidence(String projectId, String targetId) throws Exception {
        ...
        if (!targetRoot.startsWith(validationRoot)
        ...
        for (Path run : runs) {
            ...
            if (!evidenceFile.startsWith(targetRoot)
                    ...
                    || Files.isSymbolicLink(evidenceFile)) {
                continue;
            }
            List<Evidence> evidence = mapper.readValue(
                    evidenceFile.toFile(), new TypeReference<List<Evidence>>() {});
            for (Evidence item : evidence) {
                values.add(new EvidenceReceipt(
                        projectId, targetId, run.getFileName().toString(),
    ...
    private static TargetSummary targetSummary(RegisteredTarget value, long revision) {
        ...
        return new TargetSummary(
                value.projectId(), target.targetId(), target.targetName(), target.targetType().name(),
                target.immutableSourceReference(), target.adapterId(),
                Availability.NOT_AVAILABLE, null, revision);
    }
