...
def git_tree():
    try: return subprocess.check_output(['git','rev-parse','HEAD^{tree}'],cwd=ROOT,text=True).strip()
...
def main()->int:
    ...
    ap.add_argument('--machine-registry',default='contracts/dd-machine-operation-schema-fixture-registry.candidate.v1.json')
    ap.add_argument('--qualification-status',default='contracts/dd-semantic-evaluator-qualification-status.candidate.v1.json')
    ap.add_argument('--qualification-receipts-dir',default='receipts/dd-semantic-evaluator-qualification')
    ...
    expected_tree=args.source_tree_sha or git_tree(); receipt_dir=ROOT/args.receipts_dir

    machine=load(ROOT/args.machine_registry)
    operation_by_dd={r.get('dd'):r.get('operation') for r in machine.get('rows',[]) if r.get('dd') and r.get('operation')}
    ...
    qreceipt_dir=ROOT/args.qualification_receipts_dir

    positive=0; nonpositive=0; notrun=0
    ...
    for row in rows:
        ...
        qref=qr.get('qualification_receipt_ref')
        qpath=Path(qref) if qref else None
        ...
        if not qpath or not qpath.is_file(): errors.append(f'{dd}:QUALIFICATION_RECEIPT_NOT_FOUND_FOR_RUNTIME')
            ...
            qreceipt=load(qpath)
            qdigest=qreceipt.get('receipt_digest')
            ...
            if qreceipt.get('source_tree_sha')!=expected_tree: errors.append(f'{dd}:QUALIFICATION_RUNTIME_TREE_MISMATCH')
        ...
        for x in inputs:
            ...
            if not str(x.get('content_digest','')).lower().replace('sha256:','').isalnum(): errors.append(f'{dd}:INPUT_EVIDENCE_DIGEST_INVALID')
