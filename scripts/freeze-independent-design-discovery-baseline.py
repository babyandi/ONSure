...
def main()->int:
    ...
    if dirty:
        raise RuntimeError('TRACKED_WORKTREE_NOT_CLEAN_BEFORE_DISCOVERY_FREEZE:'+dirty.replace('\n','|'))
    ...
    for rel in files:
        ...
        head_bytes=subprocess.check_output(['git','show',f'HEAD:{rel}'],cwd=ROOT)
        if working_bytes!=head_bytes: raise RuntimeError(f'WORKTREE_HEAD_CONTENT_MISMATCH:{rel}')
    ...
    receipt={'contract':'ONSURE_INDEPENDENT_DISCOVERY_FROZEN_BASELINE_V2','git_commit_sha':head,'git_tree_sha':tree,'tracked_worktree_clean':True,'source_count':len(rows),'authority_population_digest':authority_digest,'sources':rows,'conclusion_leakage_count':0,'reviewer_must_read_bundle_only':True,'repository_browsing_during_blind_wave_allowed':False,'github_actions_authority':False,'final_claim_allowed':False}
    (OUT/'freeze-receipt.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    print(json.dumps({'git_commit_sha':head,'git_tree_sha':tree,'source_count':len(rows),'authority_population_digest':authority_digest,'decision':'FROZEN_BASELINE_READY_NONFINAL'},ensure_ascii=False)); return 0
if __name__=='__main__':
    try: raise SystemExit(main())
    except (OSError,subprocess.CalledProcessError,RuntimeError,UnicodeDecodeError) as e: print(f'ONSURE_DISCOVERY_FREEZE_FAIL {e}',file=sys.stderr); raise SystemExit(1)
