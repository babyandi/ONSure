...
extract_run_root() {
  local log="$1"
  awk '/^LOCAL_ASSURANCE_NONFINAL_PASS / {sub(/^LOCAL_ASSURANCE_NONFINAL_PASS /, ""); value=$0} END {print value}' "$log"
}
...
run_and_capture() {
  ...
  [[ -n "$run_root" && -d "$run_root" ]] || {
    echo "LOCAL_ASSURANCE_TWICE_FAIL NONFINAL_RUN_ROOT_NOT_REPORTED" >&2
    exit 86
  }
  printf '%s\n' "$run_root"
}
...
# nonfinal receipts must remain valid against their own per-run ledger binding.
bash "$ROOT/scripts/verify-local-assurance.sh" "$RUN1"
...
printf 'LOCAL_ASSURANCE_TWICE_NONFINAL_PASS %s %s\n' "$RUN1" "$RUN2"
