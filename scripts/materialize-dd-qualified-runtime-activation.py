...
def main()->int:
    ...
    if rc:
        print(json.dumps({'contract':'ONSURE_DD_RUNTIME_ACTIVATION_MATERIALIZATION_V1','blocking_reasons':['DD_EVALUATOR_QUALIFICATION_40_OF_40_NOT_PROVEN'],'decision':'HOLD_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 75
    ...
    for r in status.get('rows',[]):
        ...
        if dd not in expected or r.get('qualification_state')!='QUALIFIED_NONFINAL' or not ref:
            print(json.dumps({'contract':'ONSURE_DD_RUNTIME_ACTIVATION_MATERIALIZATION_V1','blocking_reasons':[f'INVALID_QUALIFICATION_STATUS:{dd}'],'decision':'HOLD_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 75
    ...
    if len(rows)!=40 or {r['dd_id'] for r in rows}!=expected:
        print(json.dumps({'contract':'ONSURE_DD_RUNTIME_ACTIVATION_MATERIALIZATION_V1','blocking_reasons':['QUALIFIED_DD_DENOMINATOR_NOT_EXACT_40'],'decision':'HOLD_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 75
    doc={
    ...
    print(json.dumps({'contract':'ONSURE_DD_RUNTIME_ACTIVATION_MATERIALIZATION_V1','qualified_count':40,'output':out.relative_to(ROOT).as_posix() if out.is_relative_to(ROOT) else str(out),'decision':'MATERIALIZED_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 0
