...
LOG="$OUT_DIR/$RUN_ID.log"; exec > >(tee "$LOG") 2>&1

echo "[ONSURE-MAIN-LOCK] mode=$MODE subject_commit=$HEAD_SHA subject_tree=$TREE_SHA"
if [[ "$MODE" == "PRE_CLEAN" ]]; then
  echo "[ONSURE-MAIN-LOCK] 1/7 current-main Java/JUnit + 160 DD fixture mechanics + activation security tests"
...
assert p.get('contract')=='ONSURE_DD_MANUAL_VERIFICATION_RECEIPT_V4'; assert p.get('receipt_digest')==d
assert p.get('source_commit_sha')==sys.argv[2]; assert p.get('source_tree_sha')==sys.argv[3]
assert p.get('verdict')=='PASS_NONFINAL_EXECUTION_MECHANICS_ONLY'
PY
  echo "$DD_RECEIPT" > "$OUT_DIR/current-dd-manual-receipt.path"

  echo "[ONSURE-MAIN-LOCK] 2/7 rebuild frozen independent-qualification binding for main commit/tree"
  ...
  echo "[ONSURE-MAIN-LOCK] 3/7 materialize qualified runtime activation"
  python3 scripts/materialize-dd-qualified-runtime-activation.py

  echo "[ONSURE-MAIN-LOCK] 4/7 stage immutable target evidence for main commit"
  ...
  echo "$STAGED_INDEX" > "$OUT_DIR/current-evidence-index.path"
  export ONSURE_DD_EVIDENCE_INDEX="$STAGED_INDEX"

  echo "[ONSURE-MAIN-LOCK] 5/7 re-run all 40 DD target semantic evaluations"
  bash scripts/run-dd-semantic-runtime-evidence.sh

  echo "[ONSURE-MAIN-LOCK] 6/7 execute post-delta closure through exact Pre-CLEAN Subject"
  set +e
  bash scripts/run-product-design-closure-post-delta.sh
  CLOSURE_RC=$?
  set -e
  # At PRE_CLEAN the expected stop is independent CLEAN. The subject must nevertheless be ready.
...
d=json.loads(p.read_text()); x=dict(d); x.pop('subject_digest',None)
calc=hashlib.sha256(json.dumps(x,ensure_ascii=False,sort_keys=True,separators=(',',':')).encode()).hexdigest()
assert d.get('subject_digest')==calc; assert d.get('source_commit_sha')==sys.argv[1]; assert d.get('source_tree_sha')==sys.argv[2]
...
PY
  ...
  echo "[ONSURE-MAIN-LOCK] 7/7 STOP_FOR_INDEPENDENT_CLEAN_A_B"
  echo "Independent CLEAN A/B must now be performed against .onsure/independent-clean/preclean-subject.json by independent verifier lineages."
  exit 82
fi

if [[ "$MODE" != "FINALIZE_LOCK" ]]; then echo "Usage: $0 PRE_CLEAN|FINALIZE_LOCK" >&2; exit 2; fi

echo "[ONSURE-MAIN-LOCK] FINAL 1/5 require prior current-main Pre-CLEAN Subject"
...
STAGED_INDEX="$(cat "$OUT_DIR/current-evidence-index.path")"
export ONSURE_DD_EVIDENCE_INDEX="$STAGED_INDEX"

echo "[ONSURE-MAIN-LOCK] FINAL 2/5 re-run full post-delta closure with CLEAN A/B present"
bash scripts/run-product-design-closure-post-delta.sh

echo "[ONSURE-MAIN-LOCK] FINAL 3/5 independent PR review must cover an ancestor PR head"
...
assert p['subject_commit_sha']==sys.argv[1]; assert p['subject_tree_sha']==sys.argv[2]
