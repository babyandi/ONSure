...
PY
  ...
  echo "[ONSURE-MAIN-LOCK] 2/8 stage the exact independently-qualified frozen subject; never regenerate it on main"
  ...
  echo "[ONSURE-MAIN-LOCK] 3/8 prove current compiled evaluator artifact still matches the qualified subject"
  python3 - <<'PY'
import hashlib,json,pathlib
m=json.loads(pathlib.Path('.onsure/dd-independent-qualification/frozen-bundle/bundle-manifest.json').read_text())
p=pathlib.Path('target/classes/kr/co/oruda/onsure/platform/BuiltInDdSemanticEvaluators.class')
assert p.is_file(),'CURRENT_EVALUATOR_CLASS_MISSING'
...
PY
  ...
  echo "[ONSURE-MAIN-LOCK] 4/8 materialize runtime activation separating execution tree from qualified subject tree"
  python3 scripts/materialize-dd-qualified-runtime-activation.py

  echo "[ONSURE-MAIN-LOCK] 5/8 stage immutable target evidence for main execution subject"
  ...
  echo "[ONSURE-MAIN-LOCK] 6/8 re-run all 40 DD target semantic evaluations"
  bash scripts/run-dd-semantic-runtime-evidence.sh

  echo "[ONSURE-MAIN-LOCK] 7/8 execute post-delta closure through exact Pre-CLEAN Subject"
...
PY
  ...
  echo "[ONSURE-MAIN-LOCK] 8/8 STOP_FOR_INDEPENDENT_CLEAN_A_B"
...
echo "[ONSURE-MAIN-LOCK] FINAL 1/5 validate main-subject Independent CLEAN A/B"
python3 scripts/validate-independent-clean-twice.py --source-commit-sha "$HEAD_SHA" --source-tree-sha "$TREE_SHA"
DD_RECEIPT="$(cat "$OUT_DIR/current-dd-manual-receipt.path")"; STAGED_INDEX="$(cat "$OUT_DIR/current-evidence-index.path")"; export ONSURE_DD_EVIDENCE_INDEX="$STAGED_INDEX"
...
set +e; ONSURE_DD_MANUAL_VERIFICATION_RECEIPT="$DD_RECEIPT" python3 scripts/issue-design-lock.py; LOCK_RC=$?; set -e
