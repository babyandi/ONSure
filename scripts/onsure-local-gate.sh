...
OUT="${ONSURE_LOCAL_GATE_OUTPUT:-$ROOT/.onsure/local-gate/$STAMP}"
...
if ! python3 -c 'import jsonschema, yaml' >/dev/null 2>&1; then
  ...
  "$OUT/validation-venv/bin/python" -m pip install \
    ...
    > "$OUT/logs/validation-dependency-install.log" 2>&1
...
if [[ "$MODE" == "full" ]]; then
  ...
  if [[ "$PROFILE" == "oruda" ]]; then
    ...
    run_step oruda-e2e mvn -B -ntp -q \
      -Dtest=io.onsure.platform.oruda.OrudaMvf001E2ETest test
  fi
  ...
  (
    ...
    npm install --ignore-scripts --no-audit --no-fund \
      > "$OUT/logs/vscode-npm-install.log" 2>&1
    npm run check > "$OUT/logs/vscode-node-check.log" 2>&1
