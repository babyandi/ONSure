...
def main()->int:
    ...
    concrete_eval=int(counts.get('dd_concrete_semantic_evaluator_code_materialized_unverified',0))
    fixture_denominator=int(counts.get('dd_qualification_fixture_case_denominator',0))
    ...
    fixture_execution=int(counts.get('dd_qualification_fixture_mechanics_executed',0))

    runtime_status=load(RUNTIME_STATUS) if RUNTIME_STATUS.exists() else {}; runtime_summary=runtime_status.get('summary',{})
    semantic_runtime=int(runtime_summary.get('pass_nonfinal_runtime_count',0))
    ...
    out={
      'contract':'ONSURE_DD_GRANULAR_VERTICAL_TRACE_VALIDATION_V6',
      'dd_count':len(rows),
      ...
      'concrete_evaluator_code_materialized_unverified_count':concrete_eval,
      'qualification_fixture_case_denominator':fixture_denominator,
      'authorized_route_execution_evidence_count':route_execution,
      'schema_validator_execution_evidence_count':schema_execution,
      'qualification_fixture_mechanics_executed_count':fixture_execution,
      'semantic_evaluator_independently_qualified_count':semantic,
      'target_semantic_runtime_evidence_count':semantic_runtime,
