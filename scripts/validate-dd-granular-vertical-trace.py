...
def main()->int:
    ...
    machine_open=sum(1 for _ in rows for k in ('operation_api_event','schema_contract','executable_test') if defaults[k]!='MATERIALIZED')
    runtime_open=sum(1 for _ in rows if defaults['runtime_evidence']!='EVIDENCED')
    ...
    out={'contract':'ONSURE_DD_GRANULAR_VERTICAL_TRACE_VALIDATION_V1','dd_count':len(rows),'machine_layer_gap_instances':machine_open,'runtime_evidence_gap_rows':runtime_open,'blocking_reasons':reasons,'decision':'PASS_NONFINAL' if not reasons else 'HOLD_NONFINAL','final_claim_allowed':False}
