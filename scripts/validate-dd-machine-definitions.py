...
def main()->int:
    reasons=[]
    for p in (REG,FIX,REQ,RES,OPS,JAVA):
        if not p.is_file(): reasons.append(f'MISSING_FILE:{p.relative_to(ROOT)}')
    if reasons:
        print(json.dumps({'contract':'ONSURE_DD_MACHINE_DEFINITION_VALIDATION_V1','blocking_reasons':reasons,'decision':'HOLD_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 36
    reg=load(REG); fixtures=load(FIX); ops=set(load(OPS)['operations']); req=load(REQ); res=load(RES); java=JAVA.read_text(encoding='utf-8')
    ...
    for r in rows:
        ...
        if not str(r.get('request_schema','')).startswith(expected_prefix+'.'): reasons.append(f'REQUEST_SCHEMA_ID_MISMATCH:{ddid}')
        if not str(r.get('result_schema','')).startswith(expected_prefix+'.'): reasons.append(f'RESULT_SCHEMA_ID_MISMATCH:{ddid}')
    ...
    java_pairs=dict(ENTRY_RE.findall(java))
    reg_pairs={r['operation']:r['dd'] for r in rows if r.get('operation') and r.get('dd')}
    missing_java=sorted(set(reg_pairs)-set(java_pairs)); extra_java=sorted(set(java_pairs)-set(reg_pairs))
    mismatch_java=sorted(op for op in set(reg_pairs)&set(java_pairs) if reg_pairs[op]!=java_pairs[op])
    ...
    absent=[t for t in fail_closed_tokens if t not in java]
    if absent: reasons.append('JAVA_FAIL_CLOSED_TOKENS_MISSING')
    out={
      'contract':'ONSURE_DD_MACHINE_DEFINITION_VALIDATION_V1',
      'dd_count':len(rows),'fixture_count':len(frows),'workflow_operation_match_count':len(registry_ops & ops),
      'java_route_match_count':len(set(reg_pairs)&set(java_pairs))-len(mismatch_java),
      'generic_request_schema':REQ.relative_to(ROOT).as_posix(),'generic_result_schema':RES.relative_to(ROOT).as_posix(),
      'execution_evidence_established':False,'github_actions_authority':False,
      'blocking_reasons':sorted(set(reasons)),'decision':'PASS_NONFINAL' if not reasons else 'HOLD_NONFINAL','final_claim_allowed':False
    }
