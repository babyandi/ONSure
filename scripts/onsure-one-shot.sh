...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "usage: bash scripts/onsure-one-shot.sh [--profile core|oruda] [--static-only]"
      exit 0
      ;;
...
[[ "$PROFILE" == "core" || "$PROFILE" == "oruda" ]] || {
  echo "ONSURE_ONE_SHOT_FAIL INVALID_PROFILE_$PROFILE" >&2; exit 64;
}

for command in git bash python3 sha256sum cmp tee; do
  command -v "$command" >/dev/null 2>&1 || {
    echo "ONSURE_ONE_SHOT_FAIL MISSING_COMMAND_$command" >&2; exit 69;
  }
done
[[ -z "$(git status --porcelain)" ]] || {
  echo "ONSURE_ONE_SHOT_FAIL WORKTREE_DIRTY_OR_UNTRACKED" >&2; exit 72;
}
HEAD_SHA="$(git rev-parse HEAD)"
[[ "$HEAD_SHA" =~ ^[0-9a-f]{40,64}$ ]] || {
  echo "ONSURE_ONE_SHOT_FAIL INVALID_GIT_HEAD" >&2; exit 72;
}
...
body = {
    "contract": "ONSURE_ONE_SHOT_ENVIRONMENT_V7",
    ...
    "tools": {name: capture(command) for name, command in {
        ...
        "maven": ["mvn", "-version"], "bwrap": ["bwrap", "--version"],
        "prlimit": ["prlimit", "--version"], "node": ["node", "--version"],
        "npm": ["npm", "--version"]
    }.items()},
}
...
set +e
ONSURE_LOCAL_GATE_OUTPUT="$OUT/local-gate" \
  bash scripts/onsure-local-gate.sh --mode "$MODE" --profile "$PROFILE" \
  > >(tee "$OUT/logs/local-gate.stdout.log") \
  2> >(tee "$OUT/logs/local-gate.stderr.log" >&2)
GATE_EXIT=$?
set -e
...
cmp "$OUT/source-start.json" "$OUT/source-end.json" >/dev/null || {
  echo "ONSURE_ONE_SHOT_FAIL SOURCE_DRIFT" >&2; exit 73;
}

python3 - "$OUT/receipts/local-gate-step.json" "$HEAD_SHA" "$PROFILE" "$MODE" \
  "$ENVIRONMENT_DIGEST" "$GATE_EXIT" "$OUT/logs/local-gate.stdout.log" \
  "$OUT/logs/local-gate.stderr.log" <<'PY'
...
body = {
    "contract": "ONSURE_ONE_SHOT_STEP_RECEIPT_V8",
    "step": "LOCAL_GATE_AUTHORITY",
    ...
    "command": ["bash", "scripts/onsure-local-gate.sh", "--mode", mode, "--profile", profile],
...
path, source, profile, mode, environment, exit_code = sys.argv[1:]
code = int(exit_code)
body = {
    "contract": "ONSURE_ONE_SHOT_RESULT_V8",
    "decision": "NON_FINAL" if code == 0 else "FAIL",
    "source_commit": source,
    "profile": profile,
    "mode": mode.upper(),
    "environment_digest": environment,
    "local_gate_exit": code,
    "local_gate_authority": True,
    "product_subrequirements": "PASS_WITH_KNOWN_GAPS" if code == 0 else "FAIL",
    "workflow_surface_parity": "PASS" if code == 0 else "FAIL",
    "critical_callpaths": "PASS" if code == 0 else "FAIL",
...
if [[ $GATE_EXIT -ne 0 ]]; then
  echo "ONSURE_ONE_SHOT_FAIL LOCAL_GATE_EXIT_$GATE_EXIT $OUT" >&2
  exit "$GATE_EXIT"
fi
