...
def files(base:Path,wid:str)->dict[str,Path]:
    return {
      'wave':base/f'{wid}.json','ledger':base/f'{wid}.candidate-ledger.json','isolation':base/f'{wid}.execution-isolation.json',
      'custody':base/f'{wid}.custody.json','envelope':base/f'{wid}.envelope.json'}

def require_docs(base:Path,wid:str)->tuple[dict,dict,dict,dict,dict,list[str]]:
    reasons=[]; ps=files(base,wid); docs={}
    for k,p in ps.items():
...
def validate_wave_structural(wid:str,w:dict,ledger:dict,iso:dict,custody:dict,envelope:dict,pair:dict,baseline:dict,baseline_sha:str,blind_sha:str)->tuple[list[str],set[str]]:
    r=[]; tokens=set()
    ...
    if custody.get('receipt_digest')!=canon(custody,'receipt_digest'):r.append('CUSTODY_DIGEST_INVALID')
        ...
        tokens.update({'sealed_result_custody','custody_ledger'})
    ...
    prompt_sha=pair.get('prompt_template_sha256')
    if not prompt_sha or envelope.get('prompt_template_sha256')!=prompt_sha:r.append('PROMPT_DIGEST_MISMATCH')
    ...
    if iso.get('sealed_envelope_only') is True and iso.get('repository_browsing_performed') is False and iso.get('issue_pr_comment_browsing_performed') is False and iso.get('opposite_wave_result_accessible') is False and iso.get('prior_conclusion_source_accessed') is False:tokens.add('blind_input_enforcement')
...
def common_control(w:dict,policy:dict,tokens:set[str])->list[str]:
    r=[]; dims=policy.get('dimensions') or []; supplied=[x for x in w.get('common_control_dimensions',[]) if isinstance(x,dict)]; by={x.get('id'):x for x in supplied if x.get('id')}
    expected={d.get('id') for d in dims}
    if len(supplied)!=len(expected) or set(by)!=expected:r.append('COMMON_CONTROL_DIMENSION_SET_NOT_EXACT')
    for dim in dims:
        ...
        required=set(dim.get('required_evidence') or []); claimed=set(item.get('mitigation_evidence') or [])
        if cls=='REQUIRES_MITIGATION':
            missing_claim=required-claimed
            unsupported=(required & claimed)-tokens
            if missing_claim:r.append('COMMON_CONTROL_MITIGATION_CLAIM_MISSING:'+did+':'+'|'.join(sorted(missing_claim)))
            if unsupported:r.append('COMMON_CONTROL_MITIGATION_UNPROVEN:'+did+':'+'|'.join(sorted(unsupported)))
        elif cls=='ALLOWED_WITH_DISCLOSURE':
            if not set(dim.get('required_evidence') or []).issubset(claimed):r.append('COMMON_CONTROL_DISCLOSURE_EVIDENCE_MISSING:'+did)
    if not w.get('common_control_attestation'):r.append('COMMON_CONTROL_ATTESTATION_MISSING')
    return r

def p1_rules(w:dict,p:dict)->list[str]:
    r=[]
    ...
    if aa!=ad:r.append('AUTHORITY_AFFECTING_P1_UNDISPOSED')
...
def main()->int:
    ...
    baseline_path=base/'frozen-baseline-receipt.json'; pair_path=base/'envelope-pair-receipt.json'
    reasons=[]
    ...
    cc=load(POLICY_CC); p1=load(POLICY_P1); blind_sha=raw_sha(BLIND_POLICY); rc=load(RECONCILER_CONTRACT)
    wave_docs={}; token_map={}; iso_map={}; envelope_map={}; custody_map={}
    for wid in WAVES:
        w,l,i,c,e,miss=require_docs(base,wid);wave_docs[wid]=w;iso_map[wid]=i;envelope_map[wid]=e;custody_map[wid]=c;reasons+=miss
        rr,tokens=validate_wave_structural(wid,w,l,i,c,e,pair,baseline,baseline_sha,blind_sha);reasons += [wid+':'+x for x in rr];token_map[wid]=tokens
    a,b=(wave_docs[WAVES[0]],wave_docs[WAVES[1]])
    ai,bi=(iso_map[WAVES[0]],iso_map[WAVES[1]])
    if a and b:
        ...
        else:token_map[WAVES[0]].add('separate_session_identity');token_map[WAVES[1]].add('separate_session_identity')
        if ai.get('workspace_identity')==bi.get('workspace_identity'):reasons.append('WORKSPACE_IDENTITY_NOT_DISTINCT')
        else:
            for wid in WAVES:token_map[wid].update({'separate_workspace','workspace_isolation'})
        for wid in WAVES:token_map[wid].add('process_isolation')
