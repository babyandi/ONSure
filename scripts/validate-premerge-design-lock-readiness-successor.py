...
def main()->int:
    ...
    rc,out,err=run([sys.executable,'scripts/reconcile-design-discovery-waves-v3.py']); rec=last_json(out); evidence['discovery_reconciler_v3']={'rc':rc,'stdout':out,'parsed':rec}
    if rc or rec.get('contract')!='ONSURE_DESIGN_DISCOVERY_RECONCILIATION_RECEIPT_V2' or rec.get('decision')!='SATURATION_CANDIDATE_NONFINAL': reasons.append('DISCOVERY_STRUCTURAL_RECONCILIATION_NOT_PASS')
    rc,out,err=run([sys.executable,'scripts/validate-human-design-authority-successor.py']); evidence['hda_22']={'rc':rc,'stdout':out}
    if rc: reasons.append('HDA_22_NOT_PASS')
    ...
    result={'contract':'ONSURE_SUCCESSOR_PREMERGE_READINESS_V3','required_dd_count':42,'required_qualification_fixture_count':173,'required_human_decision_denominator':22,'required_discovery_reconciler_contract':'ONSURE_DESIGN_DISCOVERY_RECONCILIATION_RECEIPT_V2','blocking_reasons':sorted(set(reasons)),'ready_for_main_merge':not reasons,'decision':'READY_FOR_MAIN_MERGE_NONFINAL' if not reasons else 'HOLD_NONFINAL','evidence':evidence,'main_merge_allowed':not reasons,'design_lock':False,'final_claim_allowed':False}
