...
def main()->int:
    ...
    base=subprocess.run([sys.executable,'scripts/validate-human-design-authority-decisions.py'],cwd=ROOT,capture_output=True,text=True)
    if base.returncode!=0: reasons.append('BASE_HDA_18_NOT_VALID')
    ...
    out={'contract':'ONSURE_HDA_SUCCESSOR_VALIDATION_V2','base_hda_required':18,'successor_additional_decisions_required':4,'human_decision_denominator':22,'blocking_reasons':sorted(set(reasons)),'decision':'PASS_NONFINAL' if not reasons else 'HOLD_NONFINAL','final_claim_allowed':False}
