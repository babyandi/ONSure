...
cd "$ROOT"
fail() { echo "OPTIONAL_ORUDA_FIXTURE_E2E_FAIL $1" >&2; exit 1; }
...
run_tests() {
  local output="$1"
  mvn -B -ntp -pl modules/onsure-adapter-oruda -am clean test | tee "$output/maven.log"
  grep -h '^Tests run:' modules/onsure-adapter-oruda/target/surefire-reports/*.txt \
    | python3 "$ROOT/scripts/normalize-surefire-summary.py" \
    | LC_ALL=C sort > "$output/test-summary.txt"
  (
    cd modules/onsure-adapter-oruda/target/classes
    find . -type f -print0 | LC_ALL=C sort -z | xargs -0 sha256sum
  ) > "$output/classes.sha256"
  (
    cd modules/onsure-adapter-oruda/target/test-classes
    find . -type f -print0 | LC_ALL=C sort -z | xargs -0 sha256sum
  ) > "$output/test-classes.sha256"
  sha256sum "$output/test-summary.txt" "$output/classes.sha256" \
    "$output/test-classes.sha256" > "$output/evidence.sha256"
}
...
mvn -B -ntp -pl modules/onsure-adapter-oruda -am -DskipTests package >/dev/null
mvn -B -ntp -pl modules/onsure-adapter-oruda dependency:build-classpath \
  -Dmdep.outputFile="$OUT/classpath.txt" >/dev/null
CP="$ROOT/modules/onsure-adapter-oruda/target/classes:$ROOT/modules/onsure-core/target/classes:$(cat "$OUT/classpath.txt")"
...
for run in execution-1 execution-2; do
  ...
  [[ -s "$OUT/$run/execution-inventory.json" ]] \
    || fail "EXECUTION_INVENTORY_MISSING_$run"
  grep -q '"assurance_class" : "SELF_VALIDATION_NONFINAL"' \
    "$OUT/$run/execution-inventory.json" || fail "ASSURANCE_BOUNDARY_MISSING_$run"
done

sha256sum \
  "$OUT/test-1/test-summary.txt" "$OUT/test-1/classes.sha256" \
  "$OUT/test-1/test-classes.sha256" "$OUT/test-2/test-summary.txt" \
  "$OUT/test-2/classes.sha256" "$OUT/test-2/test-classes.sha256" \
  ...
  > "$OUT/optional-oruda-fixture-e2e-lock.sha256"

cat > "$OUT/result.json" <<EOF
{
  "contract": "ONSURE_OPTIONAL_ORUDA_FIXTURE_TWO_RUN_RESULT_V1",
...
EOF
sha256sum "$OUT/result.json" >> "$OUT/optional-oruda-fixture-e2e-lock.sha256"
printf 'ONSURE_OPTIONAL_ORUDA_FIXTURE_TWO_RUN_PASS_NONFINAL %s\n' "$OUT"
