...
def resolve_evidence(index_path:Path,index:dict,value:str)->Path:
    p=Path(value)
...
def main()->int:
    ...
    ap.add_argument('--raw',required=True); ap.add_argument('--target-identity',required=True); ap.add_argument('--execution-principal',required=True); ap.add_argument('--execution-environment',required=True)
    ap.add_argument('--evidence-index',default=os.environ.get('ONSURE_DD_EVIDENCE_INDEX','.onsure/dd-runtime/evidence-index.json'))
    ap.add_argument('--receipts-dir',default='receipts/dd-semantic-runtime-evidence'); ap.add_argument('--status',default='contracts/dd-semantic-runtime-evidence-status.candidate.v1.json'); args=ap.parse_args()
    ...
    raw=load(raw_path); index=load(index_path); activation=load(ROOT/'.onsure/dd-runtime/activation.json')
    ...
    if activation.get('contract')!='ONSURE_DD_QUALIFIED_RUNTIME_ACTIVATION_V2': reasons.append('ACTIVATION_CONTRACT_NOT_V2')
    ...
    if reasons:
        print(json.dumps({'contract':'ONSURE_DD_RUNTIME_EVIDENCE_MATERIALIZATION_V3','blocking_reasons':reasons,'decision':'HOLD_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 76
    ...
    for rr in sorted(raw_rows,key=lambda x:x['dd_id']):
        ...
        rel=(Path(args.receipts_dir)/f'{dd}.json').as_posix(); (ROOT/rel).write_text(json.dumps(receipt,ensure_ascii=False,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    ...
    if reasons:
        print(json.dumps({'contract':'ONSURE_DD_RUNTIME_EVIDENCE_MATERIALIZATION_V3','blocking_reasons':sorted(set(reasons)),'decision':'HOLD_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 76
    status={'contract':'ONSURE_DD_SEMANTIC_RUNTIME_EVIDENCE_STATUS_V3','status':'RUNTIME_EXECUTED_NONFINAL' if notrun==0 else 'PARTIALLY_EXECUTED_NONFINAL','rows':status_rows,'summary':{'dd_count':40,'not_run_count':notrun,'pass_nonfinal_runtime_count':positive,'nonpositive_runtime_count':nonpositive},'source_commit_sha':commit,'source_tree_sha':tree,'qualified_subject_tree_sha':qualified_tree,'target_identity':args.target_identity,'evidence_index_path':str(index_path),'github_actions_authority':False,'final_claim_allowed':False}
    ...
    print(json.dumps({'contract':'ONSURE_DD_RUNTIME_EVIDENCE_MATERIALIZATION_V3','qualified_subject_tree_sha':qualified_tree,'execution_tree_sha':tree,'pass_nonfinal_runtime_count':positive,'nonpositive_runtime_count':nonpositive,'not_run_count':notrun,'decision':verdict,'final_claim_allowed':False},sort_keys=True)); return 0 if positive==40 else 76
