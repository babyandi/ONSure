...
def main()->int:
    ...
    if reasons:
        print(json.dumps({'contract':'ONSURE_DD_RUNTIME_EVIDENCE_MATERIALIZATION_V2','blocking_reasons':reasons,'decision':'HOLD_NONFINAL','final_claim_allowed':False},sort_keys=True)); return 76
    ...
    for rr in sorted(raw_rows,key=lambda x:x['dd_id']):
        ...
        activation_row=act[dd]; qref=activation_row.get('qualification_receipt_ref'); qpath=Path(qref); qpath=qpath if qpath.is_absolute() else ROOT/qpath
        qreceipt=load(qpath)
    ...
    status={
      ...
      'source_commit_sha':commit,'source_tree_sha':tree,'target_identity':args.target_identity,'evidence_index_path':str(index_path),'github_actions_authority':False,'final_claim_allowed':False
    }
    ...
    print(json.dumps({'contract':'ONSURE_DD_RUNTIME_EVIDENCE_MATERIALIZATION_V2','pass_nonfinal_runtime_count':positive,'nonpositive_runtime_count':nonpositive,'not_run_count':notrun,'decision':verdict,'final_claim_allowed':False},sort_keys=True)); return 0 if positive==40 else 76
