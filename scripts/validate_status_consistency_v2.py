...
def main() -> int:
    ...
    if cases.get("contract") != "ONSURE_VALIDATION_CASE_REGISTRY_V1":
    ...
    for state, count in impl_counts.items():
        if trace.get("summary", {}).get(state.lower()) != count:
            errors.append(f"TRACE_SUMMARY_MISMATCH:{state}")
    for state, count in verify_counts.items():
        if trace.get("summary", {}).get("verification", {}).get(state.lower()) != count:
            errors.append(f"TRACE_VERIFICATION_SUMMARY_MISMATCH:{state}")
    expected_matrix = {capability_key(key): item.get("implementation_status")
    ...
    sub_ids = {item.get("id") for item in sub_items if isinstance(item, dict)}
    if len(sub_items) != len(sub_ids):
        errors.append("PRODUCT_SUBREQUIREMENT_COUNT_OR_DUPLICATE")
    ...
    expected_coverage = {
        ...
        "workflow_surfaces": 3,
        "product_process_stages": len(stages),
    ...
    sub_status = verification.get("legacy_product_decomposition", {})
    if sub_status.get("requirement_count") != len(sub_items):
    ...
    expected_failure = dict(counts)
    expected_failure["all_registered_failure_injections"] = total
    ...
    for field, expected in counts.items():
        if field != "design_process_lineage_cases" and additional.get(field) != expected:
    ...
    report = {
        "contract": "ONSURE_STATUS_CONSISTENCY_REPORT_V15",
        ...
        "all_registered_failure_injections": total,
