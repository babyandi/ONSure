...
log '6/12 run requirement/design/DD machine/evaluator/runtime/human validators'
set +e
...
python3 scripts/validate-dd-semantic-runtime-evidence.py --require-all-positive-current > "$CAND/dd-semantic-runtime-evidence.log" 2>&1; DD_RUNTIME_RC=$?
...
log '10/12 aggregate blocker dimensions without allowing self-validation to override evaluator qualification, target runtime, independent or human gates'
