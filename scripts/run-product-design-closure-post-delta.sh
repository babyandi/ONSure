...
log '0/14 validate reconciled design-document authority registry'
python3 scripts/validate-design-document-authority.py | tee "$CAND/design-document-authority.log"

log '1/14 validate uncontaminated independent Design Discovery saturation evidence'
python3 scripts/validate-design-discovery-saturation.py | tee "$CAND/design-discovery-saturation.log"

log '2/14 materialize exact design inventory and verify two-run reconstructability'
...
log '3/14 materialize post-reconciliation authority with raw SHA-256'
...
s=m['review_summary']; eu=s.get('eligible_unreviewed_count',s.get('unreviewed_count',0)); ed=s.get('eligible_disputed_count',s.get('disputed_count',0))
if eu or ed:
...
log '4/14 generate Product Design EPOCH 0003 twice and compare deterministic denominator'
...
log '5/14 scan candidate EPOCH directly; historical live Requirement Universe is not mutated'
...
log '6/14 run requirement/design/DD machine/evaluator/runtime/human validators'
set +e
...
python3 scripts/validate-dd-semantic-runtime-evidence.py --require-all-pass > "$CAND/dd-runtime-evidence.log" 2>&1; DD_RUNTIME_RC=$?
python3 scripts/validate-human-design-authority-decisions.py > "$CAND/human-design-authority.log" 2>&1; HUMAN_RC=$?
set -e

log '7/14 execute local assurance twice as reproducibility evidence only'
set +e
bash scripts/run-local-assurance-twice.sh > "$CAND/local-reproducibility-twice.log" 2>&1; LOCAL_TWICE_RC=$?
set -e

log '8/14 run pre-CLEAN candidate preflight excluding only independent CLEAN'
set +e
python3 scripts/validate-product-design-candidate-preflight.py --exclude-independent-clean > "$CAND/candidate-preflight.log" 2>&1; PRECLEAN_PREFLIGHT_RC=$?
set -e

log '9/14 aggregate provisional closure; only independent CLEAN may remain before subject freeze'
python3 - "$FINAL_REQ_RC" "$COVERAGE_RC" "$DD_MACHINE_RC" "$GRANULAR_RC" "$DD_QUAL_RC" "$DD_RUNTIME_RC" "$HUMAN_RC" "$LOCAL_TWICE_RC" "$PRECLEAN_PREFLIGHT_RC" <<'PY'
...
fr,cv,dm,gr,dq,dr,hu,lr,pf=map(int,sys.argv[1:]); blockers=[]
...
if dr: blockers.append('DD_SEMANTIC_TARGET_RUNTIME_EVIDENCE_NOT_40_OF_40_PASS_NONFINAL_CURRENT')
if hu: blockers.append('HUMAN_DESIGN_AUTHORITY_DECISIONS_OPEN')
if lr: blockers.append('LOCAL_REPRODUCIBILITY_TWICE_NOT_PASS')
if pf: blockers.append('CANDIDATE_NATIVE_PRECLEAN_PREFLIGHT_NOT_PASS')
...
log '10/14 materialize exact Pre-CLEAN Subject; fails unless CLEAN is the only remaining blocker'
python3 scripts/materialize-independent-clean-preclean-subject.py | tee "$CAND/preclean-subject.log"

log '11/14 validate tracked Independent CLEAN A/B against exact Pre-CLEAN Subject'
set +e
python3 scripts/validate-independent-clean-twice.py > "$CAND/independent-clean-twice.log" 2>&1; INDEP_CLEAN_RC=$?
set -e

log '12/14 execute full candidate-native preflight including Independent CLEAN'
set +e
python3 scripts/validate-product-design-candidate-preflight.py > "$CAND/candidate-preflight.log" 2>&1; PREFLIGHT_RC=$?
set -e

log '13/14 aggregate final closure after CLEAN validation'
python3 - "$FINAL_REQ_RC" "$COVERAGE_RC" "$DD_MACHINE_RC" "$GRANULAR_RC" "$DD_QUAL_RC" "$DD_RUNTIME_RC" "$HUMAN_RC" "$LOCAL_TWICE_RC" "$INDEP_CLEAN_RC" "$PREFLIGHT_RC" <<'PY'
import json,pathlib,sys,datetime
c=pathlib.Path('.onsure/requirement-universe/epoch-0003-candidate')
s=json.loads((c/'requirement-universe-snapshot.json').read_text()); t=json.loads((c/'global-trace-scan-report.json').read_text()); r=json.loads((c/'product-design-reverse-orphan-scan-report.json').read_text())
inv=json.loads(pathlib.Path('.onsure/design-baseline/design-artifact-inventory.json').read_text()); rec=json.loads(pathlib.Path('.onsure/design-baseline/reconstructability-receipt.json').read_text())
fr,cv,dm,gr,dq,dr,hu,lr,ic,pf=map(int,sys.argv[1:]); blockers=[]
if t['orphans']['p0']: blockers.append('P0_FORWARD_ORPHANS')
if r['stale_reference_count']: blockers.append('STALE_REVERSE_REFERENCES')
if fr: blockers.append('FINAL_PRODUCT_REQUIREMENT_VALIDATION_NOT_PASS')
if cv: blockers.append('DESIGN_COVERAGE_NOT_PASS')
...
if inv.get('missing_registered_paths'): blockers.append('DESIGN_ARTIFACT_INVENTORY_INCOMPLETE')
if not rec.get('deterministic_two_run'): blockers.append('DESIGN_BASELINE_NOT_RECONSTRUCTABLE')
pre=json.loads(pathlib.Path('.onsure/independent-clean/preclean-subject.json').read_text())
receipt={'contract':'ONSURE_POST_RECONCILIATION_PRODUCT_DESIGN_CLOSURE_RECEIPT_V8','generated_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'execution_method':'LOCAL_OR_AUTOPILOT_EXPLICIT_RUN_ONLY','github_actions_authority':False,'epoch':'EPOCH::REQUIREMENT::0003::CANDIDATE','requirement_count':len(s['requirement_ids']),'requirement_manifest_digest':s['requirement_manifest_digest'],'authority_population_digest':s['authority_document_population_digest'],'preclean_subject_digest':pre.get('subject_digest'),'coverage_digest':pre.get('coverage_digest'),'design_artifact_population_digest':inv['population_digest'],'design_reconstructable':rec['deterministic_two_run'],'dd_count':len([x for x in s['requirement_ids'] if x.startswith('DD-')]),'forward_p0_orphans':len(t['orphans']['p0']),'forward_p1_orphans':len(t['orphans']['p1']),'reverse_stale_references':r['stale_reference_count'],'validator_rcs':{'final_requirement':fr,'coverage':cv,'dd_machine_definition':dm,'granular_trace':gr,'dd_evaluator_qualification':dq,'dd_target_runtime_evidence':dr,'human_authority':hu,'local_reproducibility_twice':lr,'independent_clean_twice':ic,'candidate_preflight':pf},'blocking_reasons':sorted(set(blockers)),'decision':'DESIGN_CLOSURE_CANDIDATE_NONFINAL' if not blockers else 'HOLD_NONFINAL','design_lock':False,'final_claim_allowed':False}
(c/'post-reconciliation-product-design-closure-receipt.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,sort_keys=True)+'\n'); print(json.dumps(receipt,ensure_ascii=False,sort_keys=True))
PY

log '14/14 live EPOCH was never replaced; no GitHub Actions authority or Design Lock/Final/GO claim is emitted here'
