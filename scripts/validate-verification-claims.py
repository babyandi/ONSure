...
def validate() -> list[str]:
    ...
    counts = count_authority.get("counts", {})
    total = count_authority.get("total")
    ...
    for section in (
        "design_coverage", "product_subrequirement_coverage", "workflow_surface_parity",
        "critical_callpath_boundary", "product_process_lineage"
    ):
    ...
    if required != verified | unverified:
        errors.append(
            f"SANDBOX_ATTACK_PARTITION_MISMATCH:missing={sorted(required-verified-unverified)}:"
...
def main() -> int:
    ...
    report = {
        "contract": "ONSURE_VERIFICATION_CLAIM_AUDIT_V5",
