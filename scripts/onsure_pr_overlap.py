...
def build_matrix(
    ...
    merge_hold = any(
        entry["decision"] != "NO_PATH_OVERLAP" and entry["state"] == "OPEN"
    ...
    return {
        ...
        "merge_decision": "HOLD_MERGE_ORDER_REQUIRED" if merge_hold else "NO_OVERLAP_HOLD",
        "required_actions": [
...
def current_matrix(output: pathlib.Path = DEFAULT_OUTPUT) -> dict[str, object]:
    ...
    return build_matrix(local_changed_paths(output), prs, relationship)
