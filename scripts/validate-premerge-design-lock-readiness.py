...
def main()->int:
    ...
    if not mp.is_file(): reasons.append('DD_MANUAL_VERIFICATION_RECEIPT_MISSING')
        ...
        if m.get('source_tree_sha')!=head: reasons.append('DD_MANUAL_VERIFICATION_HEAD_MISMATCH')
        ...
        if not c.get('qualification_fixture_mechanics_established'): reasons.append('DD_160_FIXTURE_MECHANICS_NOT_PROVEN')
    ...
    out={'contract':'ONSURE_PREMERGE_DESIGN_LOCK_READINESS_V1','branch':branch,'head_sha':head,'blocking_reasons':sorted(set(reasons)),'decision':'READY_FOR_MAIN_MERGE_NONFINAL' if not reasons else 'HOLD_NONFINAL','main_merge_allowed':not reasons,'design_lock':False,'github_actions_authority':False,'final_claim_allowed':False}
