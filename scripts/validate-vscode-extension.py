...
def main() -> int:
    ...
    if package.get("main") != "./extension.js": errors.append("EXTENSION_MAIN_INVALID")
    if package.get("engines", {}).get("vscode") is None: errors.append("VSCODE_ENGINE_MISSING")

    source = source_file.read_text(encoding="utf-8")
    for token in (
        "context.secrets.get", "context.secrets.store", "Authorization",
        "127\\.0\\.0\\.1", "localhost", "SELF_VALIDATION_NONFINAL",
        "independent_otester", "openLastArtifact"
    ):
        if token not in source: errors.append(f"SOURCE_TOKEN_MISSING:{token}")
    ...
    node_state = "NOT_RUN"
    if node:
        result = subprocess.run([node, "--check", str(source_file)], text=True, capture_output=True, check=False)
        node_state = "PASS" if result.returncode == 0 else "FAIL"
        if result.returncode != 0: errors.append("NODE_SYNTAX_FAIL:" + result.stderr[-500:])
...
def finish(errors: list[str], node_state: str) -> int:
    report = {
        "contract": "ONSURE_VSCODE_EXTENSION_STATIC_REPORT_V1",
