...
if [[ "$MODE" == "PRE_CLEAN" ]]; then
  ...
  echo "ONSURE_MAIN_SUCCESSOR_HOLD MAIN_PRECLEAN_42_STRUCTURAL_DISCOVERY_INTEGRATION_PENDING_AFTER_RUNTIME" >&2
  exit 82
fi
if [[ "$MODE" == "FINALIZE_LOCK" ]]; then
  echo "ONSURE_MAIN_SUCCESSOR_HOLD FINALIZE_LOCK_DISABLED_UNTIL_MAIN_42_RUNTIME_STRUCTURAL_DISCOVERY_AND_NEW_MAIN_CLEAN_A_B_EXIST" >&2
  exit 82
fi
