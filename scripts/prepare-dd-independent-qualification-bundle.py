...
def main()->int:
    ...
    reasons=[]
    if manual.get('contract')!='ONSURE_DD_MANUAL_VERIFICATION_RECEIPT_V4': reasons.append('DD_MANUAL_RECEIPT_CONTRACT_NOT_V4')
    ...
    compiled={x.get('path'):x.get('sha256') for x in manual.get('compiled_artifacts',[]) if isinstance(x,dict)}
    evaluator_class=ROOT/EVALUATOR_CLASS
    evaluator_artifact_sha=sha256(evaluator_class) if evaluator_class.is_file() else None
    ...
    obligation_sha=sha256(ROOT/'contracts/dd-semantic-evaluator-registry.candidate.v1.json')
    manifest={
      'contract':'ONSURE_DD_INDEPENDENT_QUALIFICATION_FROZEN_BUNDLE_V2',
      'generated_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),
      'source_commit_sha':commit,
      'source_tree_sha':tree,
      'artifact_population_digest':population,
      'evaluator_artifact_path':EVALUATOR_CLASS,
      'evaluator_artifact_sha256':evaluator_artifact_sha,
      'obligation_registry_sha256':obligation_sha,
      ...
      'reviewer_constraints':[
        ...
        'Synthetic mechanics receipts are prerequisite evidence only and cannot themselves be qualification evidence.',
        ...
        'Every receipt must bind evaluator_artifact_sha256 and obligation_registry_sha256 to this manifest.',
        'Any commit, tree, compiled evaluator artifact or artifact-population change invalidates this frozen bundle.',
