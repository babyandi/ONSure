...
상태: `UBUNTU_SYSTEMD_PRIMARY_RHEL_COMPATIBILITY_CANDIDATE_NONFINAL`

선택된 운영 후보는 컨테이너가 아닌 Ubuntu 24.04 LTS 단독 서버다. RHEL 계열 정의는 호환성
후보로 유지한다. 기존 경로 호환성을
...
RHEL 호환성 후보를 별도로 검사할 때만 `package_onsure_rhel.sh`와
`validate_onsure_rhel_package.py`를 사용한다.
